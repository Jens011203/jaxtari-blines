...
#helper fonk ekliyoruz
def unwrap_kangaroo_state(env_state):
    """
    Unwrap JAXAtari wrapper states until we reach the real KangarooState.

    Observed structure:
    LogState -> atari_state
    ObjectCentricState -> atari_state
    AtariState -> env_state
    KangarooState
    """

    state = env_state

    while not hasattr(state, "player"):
...
def single_run(config: dict):
    ...
    #network_params = network.init(network_key, env.observation_space().sample(obs_sample_key1).squeeze()[None, ...])
    #burayı kendim eklşyorum sample ları ve networkparamsı, input bpyutuna göre parametere oluşturduk
    sample_obs = env.observation_space().sample(obs_sample_key1).squeeze()[None, ...]
    sample_rm_state = jnp.zeros((1,), dtype=jnp.int32)
    sample_obs = append_rm_state(sample_obs, sample_rm_state)

    network_params = network.init(network_key, sample_obs)
    #sample pbs actor ve sample obs critic ekliyoruz
    sample_obs_actor = env.observation_space().sample(obs_sample_key2).squeeze()[None, ...]
    sample_obs_actor = append_rm_state(sample_obs_actor, sample_rm_state)

    sample_obs_critic = env.observation_space().sample(obs_sample_key3).squeeze()[None, ...]
    sample_obs_critic = append_rm_state(sample_obs_critic, sample_rm_state)
    agent_state = TrainState.create(
        ...
        params=AgentParams(
            ...
            #actor ve critic params vardı silip alttakileri ekledik
            actor_params=actor.init(
                actor_key,
                network.apply(network_params, sample_obs_actor)
            ),
            critic_params=critic.init(
                critic_key,
                network.apply(network_params, sample_obs_critic)
            ),
        ),
        tx=optax.chain(
    ...
    def _generate_single_final_video(mods_config, video_label, video_index=0):
        ...
        rm_state = jnp.array(RM_NORMAL, dtype=jnp.int32)
        obs_with_rm = append_rm_state(obs[None, ...], rm_state[None])[0]
        ...
        for step in range(max_steps):
            ...
            old_kangaroo_state = unwrap_kangaroo_state(old_env_state)

            obs, env_state, reward, terminated, truncated, info = env.step(env_state, action)
            done = jnp.logical_or(terminated, truncated)
            obs = obs.squeeze()

            new_kangaroo_state = unwrap_kangaroo_state(env_state)

            progress_zone_up, score_increased, lost_life, fruit_collected, level_completed, new_zone = get_kangaroo_events_jax(
                old_kangaroo_state,
                new_kangaroo_state,
            )

            rm_state = update_kangaroo_rm_state_jax(
                rm_state,
                progress_zone_up,
                score_increased,
                lost_life,
            ...
            rm_state = jnp.where(done, RM_NORMAL, rm_state)
        ...
        if len(frames) > 0:
            ...
            video_dir = os.path.join("videos", "kangaroo_rm")
            os.makedirs(video_dir, exist_ok=True)

            video_path = os.path.join(
                video_dir,
                f'kangaroo_rm_seed{config["SEED"]}_{video_label}.mp4'
            )

            # frames currently has shape (N, 3, H, W), convert back to (N, H, W, 3)
            frames_for_video = np.transpose(frames, (0, 2, 3, 1))

            imageio.mimsave(video_path, frames_for_video, fps=30)

            print(f"Video saved locally to: {video_path}")
            print(f"Final video return ({video_label}): {total_reward:.1f}")
    ...
    #nextrmstate nextobs ekledim
    #her enivornmet icin rm state baslatıyoruz ve normal pong obs un sonuna rm state ekliyoruz
    next_rm_state = jnp.zeros((config["NUM_ENVS"],), dtype=jnp.int32)
    next_obs = append_rm_state(next_obs, next_rm_state)
    ...
    def step_once(carry, step, env_step_fn):
        #bunun içine rm_state ekledik
        #cünkü rm state her stepte değişebilir
        agent_state, obs, done, key, env_state, rm_state = carry
        action, logprob, value, key = get_action_and_value(agent_state, obs, key)

        old_env_state = env_state
        old_kangaroo_state = unwrap_kangaroo_state(old_env_state)

        raw_next_obs, new_env_state, reward, next_done, info = env_step_fn(env_state, action)

        new_kangaroo_state = unwrap_kangaroo_state(new_env_state)
        ...
        next_rm_state = update_kangaroo_rm_state_jax(
            rm_state,
            progress_zone_up,
            score_increased,
            lost_life,
        ...
        next_rm_state = jnp.where(next_done, RM_NORMAL, next_rm_state)
        ...
        return (
            (agent_state, next_obs, next_done, key, new_env_state, next_rm_state),
            (storage, info),
        )

    def rollout(agent_state, next_obs, next_done, key, env_state,rm_state, step_once_fn, max_steps):
        #rollout: agent plays in enviornement and collects data info
        # we carry the part: (agent_state, next_obs, next_done, key, env_state, rm_state)
        #carry is the data that have to go from onestep to another step
        (agent_state, next_obs, next_done, key, env_state, rm_state), (storage, info) = jax.lax.scan(
            step_once_fn,
            (agent_state, next_obs, next_done, key, env_state, rm_state),
            (),
            max_steps,
        )
        return agent_state, next_obs, next_done, storage, key, env_state, rm_state, info
    ...
    for iteration in range(1, config["NUM_ITERATIONS"] + 1):
        ...
        agent_state, next_obs, next_done, storage, key, env_state,next_rm_state ,info = rollout(
            agent_state, next_obs, next_done, key, env_state,next_rm_state
        )
        ...
        if iteration % 10 == 0 or iteration == 1:
            rm_counts=jnp.bincount(next_rm_state,length=NUM_RM_STATES)
            print(
                f"Iteration {iteration:4d} | "
                f"global_step={global_step:7d} | "
                f"avg_return={metrics['charts/avg_episodic_return']:.2f} | "
                f"avg_length={metrics['charts/avg_episodic_length']:.2f} | "
f"rm_counts={rm_counts.tolist()} | "
